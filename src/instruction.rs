...
impl Instruction {
    pub fn try_from_raw_bytecode(raw_bytecode: &[u16], offset: usize) -> Result<Option<(Self, usize)>, InstructionParsingError>  {
        ...
        let (length, branch_target) = match opcode_byte {
            ...
            0x02 | 0x05 | 0x08 | 0x13 | 0x15 | 0x16 | 0x19 | 0x1A | 0x1C | 0x1F | 0x20 | 0x22 | 0x23 | 0x2D..=0x31 | 0x44..=0x6D | 0x90..=0xAF | 0xD0..=0xE2 | 0xFE | 0xFF => (2, None),
            0x03 | 0x06 | 0x09 | 0x14 | 0x17 | 0x1B | 0x24..=0x26 | 0x6E..=0x72 | 0x74..=0x78 | 0xFC | 0xFD => (3, None),
            0xFA | 0xFB => (4, None),
            0x18 => (5, None),
            0x28 => (1, Some(immediate_args as i8 as i32)),
            0x29 => (2, Some(immediate_args as i16 as i32)),
            0x2A => (3, Some(concat_words!(raw_bytecode[1], raw_bytecode[2]) as i32)),
            0x2B | 0x2C => (3, Some(concat_words!(raw_bytecode[1], raw_bytecode[2]) as i32)),
            0x32..=0x3D => (2, Some(raw_bytecode[1] as i16 as i32)),
